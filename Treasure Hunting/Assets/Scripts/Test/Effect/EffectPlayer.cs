...
public class EffectPlayer : MonoBehaviour
{
    ...
    //エフェクト用変数
    //ダッシュ時の土埃エフェクトの変数
    private GameObject DushDust;
    //土埃のエフェクトを再生・停止させるためのパーティクルシステム
    [SerializeField]
    private ParticleSystem DushDustEffect;
    ...
    void Start()
    {
        ...
        //土埃のエフェクトを見つけてくる
        DushDust = GameObject.Find("DushDust");
        //最初は土埃のエフェクトを非アクティブ
        DushDust.SetActive(false);
    }
        ...
        if (gameManagerScript.quitPanel.activeSelf == false)
        {
            ...
            if (isMoveMode == true)
            {
                ...
                if (isMaxStamina == false)
                {
                    ...
                    //走ってる間は土埃のエフェクトをアクティブに
                    DushDust.SetActive(true);
                    //走ってる間に土埃のエフェクトを再生
                    DushDustEffect.Play();
